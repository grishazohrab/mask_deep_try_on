...
def un_normalize(img: torch.tensor, mean: list, std: list):
    ...
    m = np.concatenate((mean[0] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1]),
                        mean[1] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1]),
                        mean[2] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1])), -1)

    s = np.concatenate((std[0] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1]),
                        std[1] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1]),
                        std[2] * np.ones(shape=[n_img.shape[0], n_img.shape[1], n_img.shape[2], 1])), -1)
...
def save_image(image_numpy, image_path, aspect_ratio=1.0):
    ...
    image_pil = Image.fromarray(image_numpy[0].astype(np.uint8))
    h, w, _ = image_numpy[0].shape

    if aspect_ratio > 1.0:
        image_pil = image_pil.resiyze((h, int(w * aspect_ratio)), Image.BICUBIC)
    if aspect_ratio < 1.0:
